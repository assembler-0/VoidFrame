    ...
    for (int y = 0; y < 10; y++) {
        for (int x = 0; x < 10; x++) {
            if (g_mouse_y + y < g_vbe_info->height && g_mouse_x + x < g_vbe_info->width) {
                g_compositor_buffer[(g_mouse_y + y) * g_vbe_info->width + (g_mouse_x + x)] = 0xFFFFFF;
            }
        }
    }
}
...
static void CompositeAndDraw() {
    ...
    uint32_t background_color = 0x333333;
    FastMemset(g_compositor_buffer, background_color, g_vbe_info->width * g_vbe_info->height * 4);

    for (Window* win = g_window_list_head; win != NULL; win = win->next) {
        if (win->needs_redraw) {
            WindowFill(win, 0xAAAAAA);
            WindowDrawRect(win, 0, 0, win->rect.width, 20, 0x000088);
            WindowDrawString(win, 5, 4, win->title, 0xFFFFFF);
...
void OnMouseMove(int x, int y, int dx, int dy) {
    ...
    if (g_focused_window && g_focused_window->is_moving) {
        g_focused_window->rect.x += dx;
        g_focused_window->rect.y += dy;
    }
}
